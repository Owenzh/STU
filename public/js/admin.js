$(document).ready(function () {
        ...
        function handSuccess(data, textStatus, jqXHR) {
            ...
            if (dataRes.count > 0) {
                ...
                for (var i = 0; postArr[i]; i++) {
                    ...
                    tr += '<td><span class="small text-muted">' + String(postArr[i]["p_date"]).substr(0, 10) + '</span></td>';
                    tr += '<td><span class="pull-right"><a href="#" name="' + postArr[i]["p_id"] + '" onclick="deletePost(this)">删除</a></span></td></tr>';
                    trArr.push(tr);
                }
                htmlStr = trArr.join("");
            } else {
    ...
    function queryUser() {
        ...
        function handSuccess(data, textStatus, jqXHR) {
            var dataRes = data;
            var usersArr = data.data;
            var list_html = $("#user_list_content");
            var htmlStr = '';
            $("#u_counts").text(dataRes.count);

            if (dataRes.count > 0) {
                var tr = '';
                var trArr = [];
                for (var i = 0; usersArr[i]; i++) {
                    tr = '<tr class="tie_row">';
                    tr += '<td>' + usersArr[i]["u_name"] + '</td>';
                    tr += '<td>' + usersArr[i]["u_gender"] + '</td>';
                    tr += '<td>' + usersArr[i]["u_role"] + '</td>';
                    tr += '<td>' + usersArr[i]["u_mail"] + '</td>';
                    tr += '<td><span class="pull-right"><a href="#" name="' + usersArr[i]["u_id"] + '" onclick="deleteUser(this)">删除</a></span></td></tr>';
                    trArr.push(tr);
                }
                htmlStr = trArr.join("");
            } else {
    ...
    $("#query_user").bind("click", queryUser);
    queryPost();
});
...
function deletePost(aTag) {
    //alert(aTag.name);
    //TODO
    var pID = aTag.name;
    $.ajax({
        type: 'get',
        url: '/doDelPost?p_id=' + pID,
        success: function () {
            $("#query_post").click();
        },
        error: commonErrorHandler
...
function deleteUser(aTag) {
    //alert(aTag.name);
    //TODO
    var uID = aTag.name;
    $.ajax({
        type: 'get',
        url: '/doDelUser?u_id=' + uID,
        success: function () {
            $("#query_user").click();
        },
        error: commonErrorHandler
